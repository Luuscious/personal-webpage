...
function init() {
    ...
    console.log(
    "Dice container:",
    diceAnimationContainer.clientWidth,
    diceAnimationContainer.clientHeight
    );
...
const diceContainer = document.getElementById(
    "craps-roll-dice-animation-container"
);

if (!diceContainer) {
    console.error("Dice animation container not found!");
    return;
}

console.log(
    "Dice container:",
    diceContainer.clientWidth,
    diceContainer.clientHeight
);
...
function updateMousePosition(e) {
    ...
    const rect = diceAnimationContainer.getBoundingClientRect();
    ...
    function onInputStart(e) {
        ...
        // Ignore anything that isn't inside the dice canvas
        if (!renderer || !renderer.domElement.contains(e.target)) {
            return;
        }
...
function calculateResult() {
    ...
    // Send the dice result to the Craps game
...
function onWindowResize() {
    ...
    if (!diceAnimationContainer) return;

    const width = diceAnimationContainer.clientWidth;
    const height = diceAnimationContainer.clientHeight;
...
function resizeDiceRenderer() {
    ...
    if (!diceAnimationContainer || !renderer || !camera) return;

    const width = diceAnimationContainer.clientWidth;
    const height = diceAnimationContainer.clientHeight;

    console.log("Resizing dice:", width, height);

    if (width === 0 || height === 0) {
        console.warn("Dice container is still hidden.");
        return;
    }
...
window.resizeDiceRenderer = resizeDiceRenderer;

window.roll3DDice = function () {
    console.log("ROLL 3D DICE CALLED");
    ...
    console.log("Dice count:", diceObjects.length);

    // Reset each die before throwing
    diceObjects.forEach((obj, i) => {
        const { body } = obj;

        body.position.set(
            (i - (diceObjects.length - 1) / 2) * 2.5,
            5,
            0
        );
    ...
    // Give the physics engine a moment to register
    // the reset, then throw the dice
    setTimeout(() => {
        diceObjects.forEach((obj) => {
